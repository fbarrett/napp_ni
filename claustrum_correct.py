        ...
        if dir == 'func':
            ...
            for subroot, subdirs, subfiles in os.walk(os.path.join(root, dir)):
                ...
                if bool(fn_atlas) and fn_conf and fn_claus:
                    ...
                    data_claus       = pd.read_csv(fn_claus,header=0, names=['clausL','insL','putL','clausR','insR','putR']))
                    ...
                    # data_conf        = data_conf_raw[list(conf_local)]

                    # generate corrected claustrum timecourse
                    conf_regs = np.concatenate((data_claus[['insL','putL']],data_conf_claus),axis=1)
                    glmL = sm.GLM(data_claus['clausL'], sm.add_constant(conf_regs))
                    glmR = sm.GLM(data_claus['clausR'], sm.add_constant(conf_regs))
                    resultsL = glmL.fit()
                    resultsR = glmR.fit()
                    ...
                    for atlas in fn_atlas.keys():
                        ...
                        # data_conf_loc = data_conf[:data_atlas.shape[0]] # make sure confounds and data are same length
                        # data_atlas_clean = nsig.clean(data_atlas.to_numpy(),
                        #                               confounds=data_conf_loc.to_numpy(),
                        #                               detrend=True,
                        #                               standardize='zscore_sample')
                        # clean the data BEFORE adding claustrum, as it's already been cleaned while regressing out ins/put
                        data_atlas_clean = nsig.clean(data_atlas.to_numpy(),
                                                      confounds=data_conf_claus.to_numpy(),
                                                      detrend=True,
                                                      standardize='zscore_sample')

                        data_atlas['clausL'] = resultsL.resid_response;  # data_claus['claustrumL_corrected']
                        data_atlas['clausR'] = resultsR.resid_response;  # data_claus['claustrumR_corrected']

                        # # add covariates, denoise
                        # data_conf_loc = data_conf[:data_atlas.shape[0]] # make sure confounds and data are same length
                        # data_atlas_clean = nsig.clean(data_atlas.to_numpy(),
                        #                               confounds=data_conf_loc.to_numpy(),
                        #                               detrend=True,
